    ...
    public void gmlRejectsAnXyResultInAThreeDimensionalCrs() {
        String gml = """
            <gml:Point xmlns:gml="http://www.opengis.net/gml/3.2"
              ...
              <gml:pos>10 100 7</gml:pos>
            </gml:Point>
            """.replace("\n", " ");
        NodeValue value = NodeValue.makeNode(gml, org.apache.jena.geosparql.implementation.datatype.GMLDatatype.INSTANCE);
        assertThrows(ExprEvalException.class, () -> new CentroidFF().exec(value));
        assertNull(evaluate("geof:centroid('" + gml + "'^^geo:gmlLiteral)"));
    }

    @Test
    public void emptyGmlReturnsAnEmptyPointInTheSourceCrs() {
        String gml = """
            '<gml:Polygon xmlns:gml="http://www.opengis.net/gml/3.2"
                srsName="http://www.opengis.net/def/crs/EPSG/0/4326"/>'^^geo:gmlLiteral
            """.replace("\n", " ");
        Node node = evaluate("geof:centroid(" + gml + ")");
        assertNotNull(node);
        assertEquals("http://www.opengis.net/ont/geosparql#gmlLiteral", node.getLiteralDatatypeURI());
